...
class OrderedList(object):
    ...
    def getCleanedList(self):
        self.deleteAlternateElements()

    def deleteAlternateElements(self):

        if self.head is None:
            print "Elements cannot be traversed"
        else:
            iterator = self.head
            while iterator != None and iterator.next != None:
                print iterator.data
                temp = iterator.next
                iterator = temp.next
...
if __name__=="__main__":
    ...
    print "Now we will be deleting the alternate element from the list"

    orderedList.getCleanedList()
